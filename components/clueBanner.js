...
export default function ClueBanner({ explanations, close, session }) {
  ...
  // explanations: [{ id, cluetext, rating, ratingcount, created_by_name, created_at }]
  ...
  const value = ratedIndexes[`rate${index}`]
    ? ratedIndexes[`rate${index}`]
    : explanation.rating;
      ...
      >
        <div className="bannerContent">
          ...
          {explanation.cluetext.split("\n").map((line, i) => (
            <p
              ...
              style={{ fontSize: '0.6em', marginBottom: '15px' }}
              key={i}
            >
              {line.split(" ").map((word, j) => (
                word.startsWith("http") ||
                word.startsWith("www.") ||
                word.startsWith("plonkit.net") ? (
                  <button
                    key={j}
        ...
        <div className="explanationFooter">
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <span className="createdBy" style={{ fontWeight: 10 }}>
              {`Criado por ${explanation.created_by_name}, ${humanTime}`}
            </span>
            &nbsp;&nbsp;|&nbsp;
            <Rating
              style={{ maxWidth: 100 }}
